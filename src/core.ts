...
const globalSelfState = handleContentVal();

const conts: Container[] = [];

// 状态管理
const createContainer = (defaultValue: any, defName = '') => {
...
const ComposeProvider: FC<any> = ({ children }) => {
  return conts.reduceRight((child, { Provider }) => createElement(Provider, null, child), children);
};


export default { ComposeProvider, createContainer, globalSelfState }